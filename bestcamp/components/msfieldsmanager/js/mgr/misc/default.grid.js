Msfm.grid.Default = function (config) {
    config = config || {};
    ...
    Ext.applyIf(config, {
        ...
        baseParams: {},
        cls: config['cls'] || 'main-wrapper msfm-grid',
        autoHeight: true,
        paging: true,
        remoteSort: true,
        autosave: true,
        ...
        viewConfig: {
            ...
            getRowClass: function (rec) {
                ...
                if (rec.data['published'] != undefined && rec.data['published'] == 0) {
                    cls.push('msfm-row-unpublished');
                }
                if (rec.data['active'] != undefined && rec.data['active'] == 0) {
                    cls.push('msfm-row-inactive');
                }
                if (rec.data['deleted'] != undefined && rec.data['deleted'] == 1) {
                    cls.push('msfm-row-deleted');
                }
                if (rec.data['required'] != undefined && rec.data['required'] == 1) {
                    cls.push('msfm-row-required');
                }
                return cls.join(' ');
            }
        },
    });
    Msfm.grid.Default.superclass.constructor.call(this, config);
...
Ext.extend(Msfm.grid.Default, MODx.grid.Grid, {
    ...
    getMenu: function (grid, rowIndex) {
        ...
        var menu = Msfm.utils.getMenu(row.data['actions'], this, ids);
...
Ext.reg('msfm-grid-default', Msfm.grid.Default);